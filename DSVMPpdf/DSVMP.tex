...
\title{DSVMP: Exploiting Dynamic Instruction Scheduling for VM-Based Code Obfuscation}
% author names and affiliations
% use a multiple column layout for up to three different
% affiliations
\author{\IEEEauthorblockN{Kaiyuan Kuang$^1$, Zhanyong Tang$^{1*}$, Xiaoqing Gong$^1$, Dingyi Fang$^1$, Xiaojiang Chen$^1$,\\
Tianzhang Xing$^1$, Guixin Ye$^1$, Jie Zhang$^1$, Zheng Wang$^2$}
\IEEEauthorblockA{$^1$School of Information Science and Technology, Northwest University, Xi'an, 710127, P.R. China.\\
$^2$School of Computing and Communications, Lancaster University, Lancaster, England.\\
...
% in the abstract
\begin{abstract}
\par Code virtualization built upon virtual machine (VM) technologies is emerging as a viable method for implementing code obfuscation to protect program’s code against unauthorized analysis. State-of-the-art VM-based protection approaches often use a fixed scheduling structure where the program follows a single, static execution path for same input. Such approaches, however, are fragile to dynamic cumulative attacks where the attacker can reuse knowledge extracted from previously seen software products to crack the code protection scheme of a new application.
\par This paper presents DSVMP, a novel VM-based anti-dynamic-cumulative protection scheme. As a departure from prior work, DSVMP implements a dynamic instruction scheduler to randomly direct the program to execute different paths without violating the program semantics across different runs. By randomly choosing the program execution paths, the application exposes diverse behavior, making it more difficult for the attacker reuse the knowledge collected from previous runs or other applications to perform attack. We also employ multiple VMs to further obfuscate the relationship between VM bytecode and the bytecode interpreter, making code attacking harder. We have implemented DSVMP  in a prototype system and evaluated it using commercial applications. Experimental results show that DSVMP performs well in terms of security and runtime overhead.
\par \emph{Keywords: Code virtualization; Dynamic Instruction Scheduling; Software structure diversity; Dynamic cumulative attack}
\end{abstract}
...
Software reverse attacks have been largely encountered in recent years. Such attacks lead to a number of undesired outcomes, including cheating in games, compromising DRM (digital rights management) systems, unauthorized software, pirated pay-tv etc. Software reverse attack is a major concern for software companies who want to protect the intellectual property of their software products. Industry is looking for techniques that can address this problem and improve the security of the software product. By making the original key code is difficult to understand, code obfuscation is a potential solution for the problem.
\par Code virtualization built upon a virtual machine (VM) is emerging as a promising way for implementing code obfuscation\cite{1Themida,2CV,3Vmprotect,5fang2011multi,6ming2011software,7wang2014tdvmp,8wang2013nislvmp}. The idea of VM-based protection is to replace the native machine code with virtual bytecodes which will then be interpreted at runtime by a set of bytecode handlers. In a VM-based protection scheme, the execution path of the obfuscated code is controlled by a virtual instruction scheduler. A typical scheduler consists of two parts, a dispatcher that determines which virtual instruction (bytecode) to fire, and an instruction handler that translates the bytecode into native machine code to be executed on the underlying hardware. This process hides the original program instructions by using virtual bytecode, making the software more difficult to be understood by the attacker.
\par Prior work on VM-based software protection has primarily focused on making the bytecode more complicate with a single scheduler and a single, fixed set of bytecode instructions. With such approaches, the security of the software depends on one assumption, the dispatcher and the bytecode instruction set cannot be cracked in most practical runtime environments. However, research has shown that is a dangerous assumption, for example, Nicolas et al.\cite{10falliere2009inside} presented a successful analysis process of the Trojan.Clampi that protected by VMProtect\cite{3Vmprotect}. An adversary can easily crack a software product by reusing the knowledge extracted from other applications which protected with the same set of VM tools. To solve the problem, it is vital important to introduce a certain degree of diversity into the program execution\cite{4collberg}.
\par This paper presents DSVMP, a novel VM-based code protection scheme to addresses the problem of dynamic cumulative attacks.  Our key insight is that it will be much more difficult for the attacker to analyze the code if the program behaves differently in different runs. We achieve this by introducing diversity into the program execution and bytecode interpretation. DSVMP exploits a flexible, multi-dispatched scheme to implement a dynamic, unpredictable scheduler. Unlike prior work where a program always follows a single, fixed execution path for the same input across different runs, the DSVMP scheduler directs the program to execute a randomly selected path when executing a protected code region. As a result, the program follows different execution paths in different runs and behaves differently. Our carefully designed scheme ensures that the program will produces a deterministic output for the same input despite the execution paths might look differently from the attacker’s perspective. To crack software protected under DSVMP, the adversary is forced to use a large number of trail runs to understand how the program algorithm works. As a result, this increases the cost for cracking a program.
\par To provide a certain degree of uncertainty, our VM instruction scheduler randomly selects an atomic handler to interpret a bytecode instruction for a set of candidate handlers which are implemented in different ways. Since the choice of handlers is randomly determined at runtime for each bytecode instruction, the program execution path is likely to be different in different runs. Our approach also employs a multi-VM scheme so that different code regions can be protected using different bytecodes and VM implementations. This further increase the cost for the attacker to analyze the software behavior or reusable knowledge extracted from other software products (as different products are likely to be protected by different VM bytecodes and implementations).
\par This paper makes the following contributions:
\begin{enumerate}
  \item It presents a dynamic scheduling structure for VM-based code obfuscation to protect software against dynamic cumulative attack.
  \item It is also the first to apply multiple VMs to enhance the structure of dynamic obfuscation, and successfully uses this innovative approach to protect the structure information of software.
  \item It demonstrates that the proposed scheme is effective in protecting real-world commercialized software applications.
\end{enumerate}
