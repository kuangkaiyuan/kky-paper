...
To protect software against dynamic cumulative attacks, it is important to have a certain degree of uncertainty and diversity during program execution~\cite{4collberg}.
...
address dynamic cumulative attacks.  Our key insight is that
it will be more difficult for the attacker to analyze the code if the
program behaves differently in different runs. We achieve this by introducing
rich uncertainty and diversity into program execution. To do so, \DSVMP
...
program algorithm works, significantly increasing the cost for code reverse-engineering.
...
\textbf{Dynamic cumulative attacks} 
...
protected using the same VM scheme) to perform attack. This is referred as dynamic cumulative attacks in this paper. 
...
\end{figure}
\subsection{Diversified Scheduling Structure \label{sec:dvs}}
...
The initial atom handler's structure is one set of functional components that
not be obfuscated, which are scheduled for execution by the dispatcher.
Traditionally, the dispatcher fetches the driver-data to decrypt and compute
out the sequences of the handler. According to the sequences, one set of
handlers is executed definitely. If the adversary traces the program
repeatedly, it is not difficult to figure out the specific
bytecode instructions.


To overcome this challenge, we redesign the initial handler to create a new
handler. There is an example to illustrate how we to improve and enhance the
handler structure. We will insert a control unit at the end of the handler,
and it used to do a random selection, return to the dispatcher or directly
execute the next handler. The new handler is shown in Figure~\ref{fig:newhandler}. ``\texttt{LOADS}" instruction
is used to fetch the extra added parameter
that is the offset value of two adjacent handlers.
...
add eax,9dH
\end{verbatim}
\caption{New handler??} \FIXME{Please explain what this piece of code is about!!!}
...
\subsection{Multi-VMs \label{sec:mvm}}
